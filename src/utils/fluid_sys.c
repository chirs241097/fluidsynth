...
new_fluid_thread (const char *name, fluid_thread_func_t func, void *data, int prio_level, int detach)
{
  ...
  if (prio_level > 0)
  {
    ...
    ret = thrd_create (thread, fluid_thread_high_prio, info);
  }
  else ret = thrd_create (thread, func, data);
  ...
  if (detach) thrd_detach (*thread);  // Release thread reference, if caller wants to detach
...
fluid_timer_run (void *data)
{
  ...
  while (timer->cont)
  {
    ...
    if (delay > 0) _thread_sleep (delay * 1000);
  }
