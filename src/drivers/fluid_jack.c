...
typedef struct
{
  ...
  _Atomic fluid_jack_audio_driver_t *audio_driver;
  _Atomic fluid_jack_midi_driver_t *midi_driver;
} fluid_jack_client_t;
...
fluid_mutex_t last_client_mutex;     /* Probably not necessary, but just in case drivers are created by multiple threads */
...
fluid_jack_client_close (fluid_jack_client_t *client_ref, void *driver)
{
  ...
  {
    fluid_thread_usleep (100000);  /* FIXME - Hack to make sure that resources don't get freed while Jack callback is active */
    return;
  }
...
new_fluid_jack_audio_driver2(fluid_settings_t* settings, fluid_audio_func_t func, void* data)
{
  ...
  int autoconnect = 0;
  int i;
  fluid_mutex_init(last_client_mutex);
...
fluid_jack_driver_process (jack_nframes_t nframes, void *arg)
{
  ...
  midi_driver = (fluid_jack_midi_driver_t*)fluid_atomic_pointer_get (&client->midi_driver);
