...
static int fluid_oss_audio_run(void* d);
static int fluid_oss_audio_run2(void* d);
...
static int fluid_oss_midi_run(void* d);
...
fluid_oss_audio_run(void* d)
{
  ...
  while (dev->cont)
  {
    ...
    {
      FLUID_LOG(FLUID_ERR, "Error writing to OSS sound device: %d",
                errno);
      break;
    }
  }
